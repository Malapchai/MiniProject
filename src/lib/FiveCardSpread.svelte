<script>
	...
	import { fetchFiveRandomCards } from '../services/tarotService';
	...
	// Fetch five tarot cards
	async function fetchCards() {
		loading = true;
		const fetchedCards = await fetchFiveRandomCards();
...
<style>
	...
	.cards-container {
		...
		margin-top: 3rem;
		z-index: 2;
		flex-wrap: wrap; /* Allows the cards to wrap if needed */
	}
	...
	.card {
		...
		color: white;
		width: 180px;
		height: 400px; /* Adjust height for 5-card layout */
	...
	margin-top: 5rem; /* Increase margin-top for more space */
	margin-bottom: 5rem; /* Keep margin-bottom for spacing from the bottom */
	z-index: 2;
}
